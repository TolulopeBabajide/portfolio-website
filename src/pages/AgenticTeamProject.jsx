        ...
        <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500/30">
            <div className="max-w-4xl mx-auto px-6 py-12">
                ...
                >
                    ...
                    <h1 className="text-4xl md:text-5xl font-bold mb-6 text-slate-100">Agentic Team Template</h1>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                                <div className="space-y-6">
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">Dev Agent</h4>
                                        <p className="text-slate-400 text-sm">Runs hourly, reads the shared backlog, selects the highest-priority ready item, and implements it on a feature branch. Enforces idempotency via heartbeat files and a 50-minute window check — a second invocation within the window is a no-op.</p>
                                    </div>
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">Code Review Agent</h4>
                                        <p className="text-slate-400 text-sm">Triggers at :15 after each dev commit. Performs structural review — JSX correctness, broken links, SEO/meta coverage, code quality — and writes a verdict to a review log. Blocks merge on critical findings; annotates warnings for human review.</p>
                                    </div>
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">QA Agent</h4>
                                        <p className="text-slate-400 text-sm">Runs at :30, after code review. Validates the full build pipeline: lint, build, href spot-check, title check, meta description, route shells, and resume PDF. Writes a DEPLOY-READY or BLOCKED verdict to the QA log with actionable failure details.</p>
                                    </div>
                                </div>
                                <div className="space-y-6">
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">Shared Backlog & Audit Trail</h4>
                                        <p className="text-slate-400 text-sm">All agents operate against a single source of truth: a markdown backlog with priority tiers (Critical → High → Medium → Low) and stage transitions (define → ready → in-progress → done). Every agent action is appended to a structured audit log with timestamps and exit codes.</p>
                                    </div>
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">Permission & Safety System</h4>
                                        <p className="text-slate-400 text-sm">Each agent has a declared write manifest. A permission-check script gates every file write — agents cannot touch files outside their manifest. Combined with idempotency checks and never-force-push rules, the system is safe to run unattended in production.</p>
                                    </div>
                                    <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50">
                                        <h4 className="text-slate-200 font-medium mb-2">MCP Integration Layer</h4>
                                        <p className="text-slate-400 text-sm">Agents communicate with external tools — GitHub, Figma, Slack, email — via MCP servers wired into the Claude Agent SDK. New integrations are added by registering an MCP server; no agent code changes required.</p>
                                    </div>
                                </div>
                            </div>
...
#           → log and stop`}</code>
                                ...
                                <div className="bg-slate-900/30 p-6 rounded-xl border border-slate-800/50 flex flex-col justify-center">
                                    <h4 className="text-slate-200 font-medium mb-2">Hourly Pipeline Cadence</h4>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                                {[
                                    ...
                                    <div key={label} className="bg-slate-900/50 p-6 rounded-xl border border-slate-800 text-center">

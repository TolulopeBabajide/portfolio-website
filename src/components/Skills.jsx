...
const skillCategories = [
    {
        ...
        icon: Brain,
        skills: ["Generative AI", "Google Gemini", "OpenAI API", "Google Genkit (LangChain-equivalent)", "RAG Pipelines", "MCP Integrations", "Pinecone (Vector DB)", "PostgreSQL", "Firestore"]
    },
    {
...
const Skills = () => {
    return (
        <section className="py-12 sm:py-24 px-4 sm:px-6 bg-slate-900/40 border-y border-slate-800/60">
            <div className="max-w-6xl mx-auto w-full">
                ...
                >
                    ...
                    <p className="text-sm sm:text-base text-slate-400 max-w-2xl mx-auto">Bridging the gap between complex technology and practical business outcomes.</p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-16 sm:mb-20">
                    {approachCategories.map((category, idx) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="bg-slate-800/20 border border-slate-700/50 p-5 sm:p-8 rounded-2xl"
                        >
                            <h3 className="text-lg sm:text-xl font-bold text-slate-100 mb-4 sm:mb-6">{category.title}</h3>
                            {category.points ? (
                                ...
                                <div className="space-y-4 sm:space-y-6">
                                    {category.items.map(item => (
                                        <div key={item.label}>
                                            ...
                                            <p className="text-slate-400 text-[10px] sm:text-xs leading-relaxed">{item.detail}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                ...
                >
                    {skillCategories.map((category) => (
                        <motion.div
                            ...
                            className="bg-slate-800/40 p-4 sm:p-6 rounded-xl border border-slate-800 hover:border-cyan-500/20 transition-all group"
                        >
                            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                                ...
                                <h3 className="text-sm sm:text-base font-bold text-slate-100">{category.title}</h3>
                            </div>
                            <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                {category.skills.map(skill => (
                                    <span key={skill} className="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-slate-900/60 rounded text-[9px] sm:text-[11px] font-medium text-slate-400 border border-slate-700/50">
                                        {skill}
                                    </span>
                                ))}
                            </div>
